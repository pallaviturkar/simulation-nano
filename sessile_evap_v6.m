...
%%
% ******************************************
% Load In Data from Sefiane and Bennace r
% @ 25 Celcius
% ******************************************
...
seconds = floor(NF_t(end)) + 10;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 2nd page
dt = 0.01;
loop = seconds./dt;
interval = 10; %
Tinf = 273.15 + 25;
T0=273.15 + 25;;  %%%%%My input
% Solid Surface parameters
% if strcmpi(stringSublayer,'Al')
% lams=237;
% elseif strcmpi(stringSublayer,'Steel'g)
% lams=50;
...
for i=2:loop
    ...
    while converge(index_not_converged) > 0.0001 & iloop < 201
        ...
        % Clausius
        Xlf = repmat(0.8922, 1, 500);
        Xgf =Xlf.*(101325/P).*exp((L(T,1)./(8.3144./mwl.*1000)).*(1./Tbf - 1./T));
        % Clausius Clayperion, gas phase mole frac of evap  180.7478
        % Vapor Pressure
        tXgf = Xlf.*(Pv(T)/P);
        ...
        delh = h - hprev;
        %*******************************************
        % Esimate the local concentrati8on as a result of Evap Flux J
        %*******************************************
        C = C.*hprev./h;
        %*******************************************
